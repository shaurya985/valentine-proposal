      ...
      <AnimatePresence mode="wait">
        {!accepted ? (
          ...
          >
            ...
            <div className="relative w-52 h-52 mb-8 rounded-2xl overflow-hidden bg-rose-50 border-4 border-white shadow-md flex items-center justify-center">
              {/* 
            ...
            {/* Proposal Heading */}
            <h1 className="font-pacifico text-4xl sm:text-5xl text-rose-800 leading-tight mb-8 drop-shadow-sm select-none">
              Will you be my Valentine? 💖
            </h1>

            {/* Interactive Response Buttons */}
            <div className="flex flex-row items-center justify-center gap-4 sm:gap-6 min-h-[140px] w-full px-2">
              <motion.button
                onClick={() => setAccepted(true)}
                style={{ scale: yesScale }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
                className="px-6 py-3 sm:px-8 sm:py-3.5 bg-gradient-to-r from-rose-500 via-pink-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 text-white font-semibold text-lg rounded-full shadow-[0_8px_20px_rgba(244,63,94,0.3)] hover:shadow-[0_10px_25px_rgba(244,63,94,0.45)] transform active:scale-95 transition-all duration-300 flex items-center justify-center gap-2 select-none cursor-pointer whitespace-nowrap"
                id="yes-btn"
              >
              ...
              {noClicks < NO_BUTTON_TEXTS.length ? (
                <motion.button
                  ...
                  className="px-5 py-2.5 sm:px-6 sm:py-3 bg-slate-200 hover:bg-slate-300 text-slate-700 font-medium text-base rounded-full border border-slate-300/30 hover:border-slate-300/60 shadow-sm active:scale-95 transition-all duration-300 select-none cursor-pointer whitespace-nowrap"
                  id="no-btn"
                >
                  {currentNoText}
                </motion.button>
